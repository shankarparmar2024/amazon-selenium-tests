"""
Test Case 1 — Search for an iPhone 17 Pro, add to cart, print price.
"""
import pytest
...
class TestIPhoneCart:

    def test_search_iphone_add_to_cart(self, driver):
        query = SEARCH_QUERIES["iphone"]
        logger.info(f"Test Case 1 started  →  query: '{query}'")
        ...
        product.take_screenshot("tc1_iphone_cart")

        assert added, (
            "Could not add iPhone to cart — Add to Cart button not found. "
            "Amazon may be showing a carrier selection or region-locked page."
        )

        after = product.get_cart_count()
        logger.success(f"iPhone added to cart  |  Cart: {before}→{after}  |  Price: {price}")
